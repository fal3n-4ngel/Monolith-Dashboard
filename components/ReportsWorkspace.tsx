...
export function ReportsWorkspace() {
  ...
  return (
    <div className="min-h-screen flex flex-col font-sans bg-[var(--bg-primary)] text-[var(--text-primary)]">
      ...
      <main className="flex-1 max-w-[1100px] w-full mx-auto p-4 sm:p-6 lg:p-8 space-y-6">
        ...
        {loading ? (
          <div className="border border-[var(--border-subtle)] bg-[var(--bg-card)] p-16 text-center text-xs text-[var(--text-secondary)] font-mono">
            <RefreshCw className="w-4 h-4 animate-spin inline mr-2" />
            Loading reports…
          </div>
        ) : reports.length === 0 ? (
          <div className="border border-[var(--border-subtle)] bg-[var(--bg-card)] p-16 text-center text-xs text-[var(--text-secondary)] font-sans">
              ...
              <div className="flex flex-wrap items-center gap-1 border-b border-[var(--border-subtle)] pb-2">
                {tabs.map((t) => (
                  <button
                    ...
                    className={`px-2.5 py-1 text-[11px] font-mono font-bold uppercase tracking-widest transition-colors cursor-pointer ${
                      activeTab === t.key
            ...
            {visibleReports.map((report) => {
              ...
              return (
                <div key={report.id} className="border border-[var(--border-subtle)] bg-[var(--bg-card)]">
                  <div className="px-4 py-3 border-b border-[var(--border-subtle)] flex items-start gap-3">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="text-[10px] font-mono text-[var(--text-secondary)] mt-0.5">{report.id}</div>
                    </div>
                  </div>
                  ...
                  >
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                      <button
                        ...
                        className="flex items-center gap-2 px-3 py-2 bg-[var(--text-primary)] text-[var(--bg-primary)] text-xs font-mono font-semibold uppercase tracking-widest hover:opacity-90 transition-opacity disabled:opacity-50 cursor-pointer"
                      >
                        {running === report.id ? (
                      ...
                      {runError[report.id] && (
                        <span className="flex items-center gap-1.5 text-[11px] font-mono text-red-700">
                          <AlertTriangle className="w-3.5 h-3.5" />
                          {runError[report.id]}
                        </span>
                      )}
                      {note[report.id] && !runError[report.id] && (
                        <span className="text-[11px] font-mono text-[var(--text-secondary)]">{note[report.id]}</span>
                      )}
                    </div>
                  </form>
                </div>
...
function FieldInput({
  ...
  const cls =
    "px-2.5 py-1.5 bg-[var(--bg-primary)] border border-[var(--border-subtle)] text-xs font-mono focus:outline-none focus:border-[var(--border-strong)] transition-colors disabled:opacity-70 disabled:cursor-not-allowed";
